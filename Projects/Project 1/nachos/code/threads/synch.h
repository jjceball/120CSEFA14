...
public:
    ...
    bool lockEqualCurrent();	// true if the current thread
...
private:
    ...
    List *queue;
    bool isLocked;           // returns true if thread is locked 
    Thread *lockThread; 
...
//
// Mailbox has variables, described below, but it is basically a lockable 
// stack where infomation can be put in and taken out by different threads
// without running into each other
//

class Mailbox {
public:
  Mailbox(char* debugName, Condition* condition, Condition* condbuf, Lock* lock, Lock* lockbuf);  // Initialize
  ~Mailbox();
...
//private variables that help with the locking
private:
  Lock* maillock;
  Condition* mailcond;
//must be able to lock both the whole data structure if the mailbox but also buf
  Lock* buflock;
  Condition* bufcond; 
  int* buffer;
  char* name;
...
//
// see mailbox as above, it is the same as above, but now instead of having one condition 

class Whale {
public:
  Whale(char* debugName);  // Initialize
  ~Whale();
  char* getName() {
    return (name); 
  }
  ...
  void Female();
  void Matchmaker();
//private variables that help with locking between three "threads" or whales
private:
  Lock* lock;
  Condition* match;  
  char* name;
