...
//----------------------------------------------------------------------
// AddrSpace::AddrSpace
//	Removed the initialization of the AddrSpace and moved it 
//	to its own method.  	
//----------------------------------------------------------------------
...
{
    Initialize(executable, TRUE); // Default 
    return 0;
}

//----------------------------------------------------------------------
// Seek for Physical Page
//
//----------------------------------------------------------------------

int SeekForPhysicalPage(TranslationEntry* vm, unsigned int vmSize, int virtualPage) 
{
    unsigned int i;
    for (i = 0; i < vmSize; i++) {
...
//----------------------------------------------------------------------
int
AddrSpace::Initialize(OpenFile *executable, bool lock)
{
    NoffHeader noffH;
    unsigned int i, size;
...
// first, set up the translation
	...
	// We aren't using real virtual memory, so we assign a physical memory page
        pageTable[i].physicalPage =  memoryManager->AllocPage(lock);
...
// then, copy in the code and data segments into memory
    ...
    {
        var = pageTable[memToStart / PageSize].physicalPage;  //page num
    ...
    if (memToSize > 0)
    {
	var = pageTable[memToStart / PageSize].physicalPage;
	...
	executable->ReadAt(&(machine->mainMemory[var*PageSize + memory]),
			memToSize, fileBegin);
    }
...
int AddrSpace::CopyCurrentSpace() {
    ...
    pageTable = new TranslationEntry[numPages]; //clears space for new memory
	...
	pageTable[i].virtualPage = i;
	newPhysicalPage = memoryManager->AllocPage(1); 
