...
void Lock::Acquire() {
    ...
    if (isLocked) {
      ...
      queue->SortedInsert(currentThread, 0 - currentThread->getPriority());    
      //put this thread (the current thread) on the wait Q for the lock;

      if (lockThread->getPriority() < currentThread->getPriority()) {
        lockThread->givePriority(currentThread->getPriority());
      }
...
void Condition::Wait(Lock* conditionLock) {
    ...
    queue->SortedInsert(currentThread, 0 - currentThread->getPriority());    //put this thread (the current thread) on the wait Q for the lock;
...
Mailbox::Mailbox(char* debugName) {
  name = debugName;
  mailcond = new Condition("mailboxcond");
  bufcond = new Condition("bufcond");
  maillock = new Lock("maillock");
  buflock = new Lock("buflock");
  inc = 0;
  buffer = new int;

  e = new Semaphore("empty", 1);
  f = new Semaphore("full", 0);
}
...
void Mailbox::Send(int message) {
  ...
  maillock->Release(); // release lock

  e->P(); // empty semaphore 

  buflock->Acquire(); // acquire lock
  while (buffer_count == 1) {
    bufcond->Wait(buflock);       // use of a buffer for the send method
  }

  buffer = &message;

  buffer_count = 1; // increase buffer

  bufcond->Signal(buflock); // signal condition
  
  buflock->Release(); // release lock
...
void Mailbox::Receive(int * message) {
  ...
  buflock->Acquire(); // acquire buffer lock
  ...
  *message = *buffer;
  buffer_count = 0; // decrease buffer count 

  bufcond->Signal(buflock); // signal buffer lock
  buflock->Release(); // release lock
  e->V(); // empty semaphore
}
